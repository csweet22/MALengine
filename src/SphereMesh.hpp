...
class SphereMesh : public GameObject
{
...
public:
    SphereMesh(float min = -2.0f, float max = 2.0f, float stepsize = 0.2f);
    void setupTexture();
    void setupVBOs();
    void setupVAO();
    void setShaderID(GLuint programID);
    float remap(float input, float iLow, float iHigh, float gLow, float gHigh);
	void GenerateMesh(int radius, int latitudes, int longitudes);
    void Draw();
    void Update();
    Camera* cam;
};
